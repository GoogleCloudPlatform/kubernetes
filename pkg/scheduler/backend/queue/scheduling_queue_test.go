...
func TestPriorityQueue_MoveAllToActiveOrBackoffQueue(t *testing.T) {
	...
	for q.backoffQ.len() != 0 {
		_, err := q.backoffQ.popBackoff()
		if err != nil {
			_, err = q.backoffQ.popErrorBackoff()
			if err != nil {
				t.Errorf("Pop from backoffQ failed: %v", err)
			}
		}
	}
	expectInFlightPods(t, q, medPriorityPodInfo.Pod.UID)
...
func TestMoveAllToActiveOrBackoffQueue_PreEnqueueChecks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var got []string
			for q.backoffQ.len() != 0 {
				queuedPodInfo, err := q.backoffQ.popBackoff()
				if err != nil {
					queuedPodInfo, err = q.backoffQ.popErrorBackoff()
					if err != nil {
						t.Errorf("Fail to pop pod from backoffQ: %v", err)
					}
				}
