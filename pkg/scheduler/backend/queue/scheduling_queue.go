...
func (p *PriorityQueue) flushBackoffQCompleted(logger klog.Logger) {
	...
	defer p.lock.Unlock()
	activated := false
	p.backoffQ.forEachBackoffCompleted(logger, func(pInfo *framework.QueuedPodInfo) {
...
func (p *PriorityQueue) Update(logger klog.Logger, oldPod, newPod *v1.Pod) {
	...
	if pInfo := p.unschedulablePods.get(newPod); pInfo != nil {
		...
		if isPodUpdated(oldPod, newPod) {
			// Pod could finish his backoff while being in unschedulablePods,
			// so shouldBeBackingoff is checked before moving the pod to backoffQ.
			if p.backoffQ.shouldBeBackingoff(pInfo) {
				p.backoffQ.add(logger, pInfo)
				p.unschedulablePods.delete(pInfo.Pod, gated)
...
func (p *PriorityQueue) requeuePodViaQueueingHint(logger klog.Logger, pInfo *framework.QueuedPodInfo, strategy queueingStrategy, event string) string {
	...
	// Pod could finish his backoff while being in unschedulablePods,
	// so shouldBeBackingoff is checked before moving the pod to backoffQ.
	if strategy == queueAfterBackoff && p.backoffQ.shouldBeBackingoff(pInfo) {
		p.backoffQ.add(logger, pInfo)
