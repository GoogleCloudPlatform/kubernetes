...
type backoffQueuer interface {
	shouldBeBackingoff(podInfo *framework.QueuedPodInfo) bool
	getBackoffTime(podInfo *framework.QueuedPodInfo) time.Time
	forEachBackoffCompleted(logger klog.Logger, fn func(pInfo *framework.QueuedPodInfo))
	...
	popErrorBackoff() (*framework.QueuedPodInfo, error)
...
func newBackoffQueue(clock clock.Clock, podInitialBackoffDuration time.Duration, podMaxBackoffDuration time.Duration) *backoffQueue {
	...
	bq.podBackoffQ = heap.NewWithRecorder(podInfoKeyFunc, bq.podsCompareBackoffCompleted, metrics.NewBackoffPodsRecorder())
	bq.podErrorBackoffQ = heap.NewWithRecorder(podInfoKeyFunc, bq.podsCompareBackoffCompleted, metrics.NewBackoffPodsRecorder())
...
// podsCompareBackoffCompleted is a less function of podBackoffQ and podErrorBackoffQ.
func (bq *backoffQueue) podsCompareBackoffCompleted(pInfo1, pInfo2 *framework.QueuedPodInfo) bool {
	bo1 := bq.getBackoffTime(pInfo1)
	bo2 := bq.getBackoffTime(pInfo2)
	return bo1.Before(bo2)
}

// shouldBeBackingoff returns true if a pod is still waiting for its backoff timer.
// If this returns true, the pod should not be re-tried.
func (bq *backoffQueue) shouldBeBackingoff(podInfo *framework.QueuedPodInfo) bool {
...
func (bq *backoffQueue) forEachBackoffCompletedWithQueue(logger klog.Logger, fn func(pInfo *framework.QueuedPodInfo), queue *heap.Heap[*framework.QueuedPodInfo]) {
	for {
		pInfo, ok := queue.Peek()
		if !ok || pInfo == nil {
			break
		...
		pod := pInfo.Pod
		if bq.shouldBeBackingoff(pInfo) {
			break
		}
		_, err := queue.Pop()
		if err != nil {
...
// forEachBackoffCompleted runs fn for all pods from podBackoffQ and podErrorBackoffQ that completed backoff.
func (bq *backoffQueue) forEachBackoffCompleted(logger klog.Logger, fn func(pInfo *framework.QueuedPodInfo)) {
	// Ensure both queues are called
	bq.forEachBackoffCompletedWithQueue(logger, fn, bq.podBackoffQ)
	bq.forEachBackoffCompletedWithQueue(logger, fn, bq.podErrorBackoffQ)
}
...
// popBackoff pops the pInfo from podBackoffQ.
func (bq *backoffQueue) popBackoff() (*framework.QueuedPodInfo, error) {
	return bq.podBackoffQ.Pop()
}

// popErrorBackoff pops the pInfo from podErrorBackoffQ.
func (bq *backoffQueue) popErrorBackoff() (*framework.QueuedPodInfo, error) {
	return bq.podErrorBackoffQ.Pop()
}
